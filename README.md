# Service Operations SQL Project

## Overview
This project analyzes service operations data to evaluate SLA performance, technician workload, and request trends using SQL.

## Business Insights

- 70% of service requests were completed within SLA
- 30% of requests remain in progress, indicating potential backlog
- Technician workload is evenly distributed across the team
- Electrical Issues and HVAC Repairs are the most frequent request types, suggesting areas for resource prioritization

## Tools Used
- SQLite
- SQL
